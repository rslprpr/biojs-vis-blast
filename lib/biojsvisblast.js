...
var width = 900,height = 40, seq_height = 10, padding = 5, side_padding = 20;
		...
		model.iterations.forEach(function(it){
			it.hits.forEach(function(hit){
				...
				hit.hsps.forEach(function(hsp_hit){
					// drawAlignment(model["query-len"], hit.len, hsp_hit, div);
					drawAlignment(model.param.sequences[0].length, hit.len, hsp_hit, div);
				})
			});
		});
	}
...
var drawAlignment = function (queryLength, hitLength, hsp, element) {
